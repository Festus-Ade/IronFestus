...
-- Using the loan table, print the top 10 account_ids based on the sum of all of 
-- their loan amounts.
SELECT sum(amount), account_id FROM bank.loan
GROUP BY account_id
...
-- select * from bank.loan;
SELECT date, count(amount) FROM bank.loan
WHERE date < 930907 
GROUP BY date
ORDER BY date DESC;
...
-- Continuing with the previous example, rank the top 10 account_ids based on their
--  difference.
